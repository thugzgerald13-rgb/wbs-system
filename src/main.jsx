...
function Login({ setAuth }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (!session && isSignup) {
        setError("Account created. Check your email if Supabase confirmation is enabled, then log in.");
        return;
      }
      setAuth({ session, user: session?.user || data.user });
      localStorage.setItem("wbs-user", JSON.stringify(session?.user || data.user));
